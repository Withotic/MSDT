...
async def main(tasks):
    ...
    while True:
        os.system('cls')
        # генерация заказа
        task=[rnd.randint(0,3)==0,rnd.randint(0,2)==0,rnd.randint(0,1)==0]
        if(not (not task[0] and not task[1] and not task[2])):
            tasks.append(task)
...
print("Каждый цикл случайно появляется клиент с заказом")
